...
mod components {
    use super::*;
    clx! {CommandHeader, div, "flex flex-col gap-2 text-center sr-only sm:text-left"}
    ...
    let script_content = format!(
        ...
        (function() {{
            const setupDialog = () => {{
                ...
                const openBtn = document.querySelector('#{trigger_id}');

                if (!dialog || !backdrop || !openBtn) {{
                ...
                document.addEventListener('keydown', (e) => {{
                    ...
                    else if (e.key === 'Escape' && dialog.getAttribute('data-state') === 'open') {{
                        e.preventDefault();
                        closeDialog();
                    }}
                }});
...
#[component]
pub fn Command(children: Children, #[prop(into, optional)] class: String) -> impl IntoView {
    let dialog_context = use_context::<CommandDialogContext>();
    let search_query_signal = RwSignal::new(String::new());
    let command_context = CommandContext { search_query_signal };
...
#[component]
pub fn CommandInput(#[prop(into, optional)] class: String) -> impl IntoView {
    ...
    view! {
        <input
            ...
            on:input=move |ev| {
                command_context.search_query_signal.set(event_target_value(&ev));
            }
            autofocus="true"
...
pub fn CommandItem(
    ...
    let merged_class = tw_merge!(
        "auto__check__icon relative flex gap-2 items-center px-2 py-1.5 text-sm rounded-sm cursor-default select-none outline-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 hover:bg-accent hover:text-accent-foreground",
        class
    );
    ...
    view! {
        <div
            ...
            data-check-space=reserve_check_space.then_some("")
